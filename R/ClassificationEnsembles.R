...
#' @param save_all_plots Allows user to save all plots and reports
#' @param scale_all_numeric_predictors_in_data Allows user to scale all numeric predictors in the data set
...
Classification <- function(data, colnum, numresamples, predict_on_new_data = c("Y", "N"), how_to_handle_strings = c(0("No strings"), 1("Strings as factors")), save_all_trained_models = c("Y", "N"),
                           save_all_plots = c("Y", "N"), scale_all_numeric_predictors_in_data = c("Y", "N"), use_parallel = c("Y", "N"), train_amount, test_amount, validation_amount) {
...
if (predict_on_new_data == "Y") {
  ...
  new_data <- new_data %>% dplyr::relocate(y, .after = dplyr::last_col()) # Moves the target column to the last column on the right
}
...
accuracy_barchart <- ggplot2::ggplot(Results, aes(x = reorder(Model, dplyr::desc(Mean_Holdout_Accuracy)), y = Mean_Holdout_Accuracy)) +
  ...
  ggplot2::ylim(0, max(Results$Mean_Holdout_Accuracy) + 0.5) +
...
holdout_vs_train_barchart <- ggplot2::ggplot(Results, aes(x = reorder(Model, dplyr::desc(holdout_vs_train)), y = holdout_vs_train)) +
  ...
  ggplot2::ylim(0, max(Results$holdout_vs_train) +0.5) +
...
duration_barchart <- ggplot2::ggplot(Results, aes(x = reorder(Model, Duration), y = Duration)) +
  ...
  ggplot2::ylim(0, max(Results$Duration) + 0.1) +
...
if (predict_on_new_data == "Y") {
  ...
  return(list(
    ...
    "Duration_barchart" = duration_barchart, "Data summary" = data_summary, "Correlation_Matrix" = correlation_marix, "Boxplots" = boxplots, "Histograms" = histograms, "Head of data" = head_data, "Head of Ensembles" = head_ensemble,
...
return(list(
  'Final_results' = Final_results, 'Barchart' = barchart, "Accuracy_Barchart" = accuracy_barchart, "holdout_vs_train_barchart" = holdout_vs_train_barchart,
  "Duration_barchart" = duration_barchart,  'Data_summary' = data_summary, 'Correlation_matrix' = correlation_marix,
  'Boxplots' = boxplots, 'Histograms' = histograms, 'Head_of_data' = head_data, 'Head_of_ensemble' = head_ensemble,
  'Summary_tables' = summary_tables, 'Accuracy_plot' = accuracy_plot, 'Total_plot' = total_plot, "Total_plot2" = total_plot2,
  "Holdout_vs_train_plot" = holdout_vs_train_plot
)
)
}
